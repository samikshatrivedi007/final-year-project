...
const StudentDashboardPage: React.FC = () => {
  ...
  useSocket(
    branch && data?.student?.course
      ? `branch:${data.student.course}:${branch}`
      : null,
    {
          ...
          >
            ...
            <div className="card">
              ...
              <div className="card-value">
                {visiblePendingAssignments.length}/{data.totalAssignments || 0}
              </div>
                ...
                <div key={t._id} className="timetable-row">
                  ...
                  <div className="timetable-subject">
                    ...
                    <div className="subject-meta">
                      {t.facultyId.name} · {t.room}
                    </div>
                  </div>
                  {isClassActive(t.dayOfWeek, t.startTime, t.endTime) &&
                    t.isLive && (
                      <button
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">
            Full Timetable — {data.student.branch} Branch
          </span>
        </div>
        {days.map((day) => {
        ...
        {visibleTimetable.length === 0 && (
          <p style={{ color: "var(--text-muted)", padding: "1rem 0" }}>
            No timetable entries for your branch yet. Contact admin.
          </p>
        )}
      </div>
    );
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              Assignments Due ({pending.length})
            </span>
          </div>
          <div className="assignment-list">
            {pending.length === 0 && (
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              Completed / Reviewed ({completed.length})
            </span>
          </div>
          <div className="assignment-list">
            {completed.length === 0 && (
                ...
                <div className="assignment-info">
                  ...
                  <div className="asgn-meta">
                    ...
                    {a.mySubmission?.grade !== undefined && (
                      ...
                      >
                        Grade: {a.mySubmission.grade}/100
                      </span>
                    )}
        ...
        >
          ...
          >
            ...
            <p style={{ color: "var(--text-muted)", marginBottom: "0.5rem" }}>
              <b>{submitModal.title}</b> · {submitModal.courseId.name}
            </p>
            <p
              style={{
                color: "var(--text-muted)",
            ...
            {submitModal.mySubmission?.fileUrl && (
              ...
              >
                <p style={{ margin: 0, fontSize: "0.85rem", color: "#065F46" }}>
                  ✓ Current submission:{" "}
                  <a
                    href={resolveFileUrl(submitModal.mySubmission.fileUrl)}
                    target="_blank"
            ...
            >
              ...
              >
                {fileName ? (
                  <>
                    <b style={{ color: "#10B981" }}>✓ {fileName}</b>
                  </>
                ) : submitModal.mySubmission ? (
            ...
            <button
              ...
              disabled={
                (!selectedFile && !submitModal.mySubmission?.fileUrl) ||
                uploadLoading ||
                submitLoading
              }
            ...
            <button
              ...
              style={{ marginTop: "0.5rem", borderColor: "var(--border)" }}
              onClick={() => setSubmitModal(null)}
            >
              Cancel
            </button>
