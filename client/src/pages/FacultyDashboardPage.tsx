...
const SERVER_BASE =
  import.meta.env.VITE_API_URL?.replace("/api", "") || "http://localhost:9000";
...
const FacultyDashboardPage: React.FC = () => {
  ...
  >({});
  const getReviewForm = (subId: string) =>
    reviewForms[subId] || { grade: "", feedback: "" };
  const setSubForm = (
    subId: string,
    ...
    setReviewForms((f) => ({
      ...f,
      [subId]: { ...getReviewForm(subId), ...patch },
    }));
  ...
  // Attendance modal
  const [attendanceModal, setAttendanceModal] = useState<TimetableEntry | null>(
    null,
  );
  const [students, setStudents] = useState<
    { _id: string; name: string; rollNo: string }[]
  >([]);
  ...
  const [courses, setCourses] = useState<
    { _id: string; name: string; code: string; branch: string }[]
  >([]);
  ...
  const fetchCourses = useCallback(async () => {
    try {
      const d = await facultyService.getCourses();
      setCourses(d);
    } catch {
      /* ignore */
    }
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    ...
    try {
      await facultyService.createAssignment(
        createForm as Parameters<typeof facultyService.createAssignment>[0],
      );
  ...
  const handleGrade = async (sub: Submission) => {
    ...
    if (!form.grade) {
      setReviewMsg("Grade is required");
      return;
    }
    ...
    try {
      ...
      setReviewMsg("");
      // Refresh submissions
      const result = await facultyService.getSubmissions(
        reviewModal.assignmentId,
      );
      setReviewSubmissions(result.submissions || []);
      await fetchDashboard();
    } catch (err: unknown) {
  ...
  const openAttendanceModal = async (entry: TimetableEntry) => {
    ...
    if (!branch) {
      alert("No branch info on this class");
      return;
    }
    try {
      const list = await facultyService.getStudentsByCourseAndBranch(
        entry.course,
        branch,
      );
      setStudents(list);
      const defaultRecords: Record<string, string> = {};
      list.forEach((s: { _id: string }) => {
  ...
  const renderDashboard = () => {
    ...
    const pending = (data.assignments || []).filter((a) =>
      a.submissions.some((s) => !s.isReviewed),
    );
    return (
      <>
          ...
          <div className="card">
            ...
            <div className="timetable-list">
              ...
              {(!data.schedule || data.schedule.length === 0) && (
                <p style={{ color: "var(--text-muted)" }}>No classes today</p>
              )}
            </div>
          </div>
                ...
                return (
                  <div key={a._id} className="assignment-row">
                    <div className="assignment-info">
                      <div className="asgn-name">
                        {unreviewedSub?.studentName || "Student"}
                      </div>
  ...
  const renderSchedule = () => {
    ...
    return (
      <div className="card">
        <div className="card-header">
          <span className="card-title">Full Schedule</span>
        </div>
        {days.map((day) => {
                    ...
                    <div key={t._id} className="timetable-row">
                      ...
                      >
                        ...
                        {status === "active" &&
                          (t.isLive ? (
                            <button
                              className="btn btn-sm badge-live"
                              style={{ border: "none", cursor: "pointer" }}
                              onClick={() => handleToggleLive(t)}
                            >
                              Stop Live
                            </button>
                          ) : (
        ...
        {(!data.schedule || data.schedule.length === 0) && (
          <p style={{ color: "var(--text-muted)", padding: "1rem 0" }}>
            No schedule entries. Admin needs to add timetable entries for your
            faculty.
          </p>
        )}
      </div>
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleCreateAssignment} className="auth-form">
              ...
              <div className="form-group">
                <label>
                  Subject (Course) <span style={{ color: "red" }}>*</span>
                </label>
                <select
                  className="form-input"
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({ ...f, branch: e.target.value }))
                  }
                  required
                >
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Roll: {sub.rollNo} · Branch: {sub.branch}
                    </div>
                    <div
                      style={{
                        color: "var(--text-muted)",
                  ...
                  <div style={{ marginTop: "0.75rem" }}>
                    ...
                    >
                      <input
                        ...
                        max={100}
                        style={{ width: 140 }}
                        value={getReviewForm(sub._id).grade}
                        onChange={(e) =>
                          setSubForm(sub._id, { grade: e.target.value })
                        }
                      />
                      <input
                        className="form-input"
                        ...
                        style={{ flex: 1 }}
                        value={getReviewForm(sub._id).feedback}
                        onChange={(e) =>
                          setSubForm(sub._id, { feedback: e.target.value })
                        }
                      />
                      <button
                        className="btn btn-primary btn-sm"
      ...
      {attendanceModal && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0 }}>
                Mark Attendance — {attendanceModal.courseId.name}
              </h3>
              <button
